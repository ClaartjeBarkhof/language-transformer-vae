...
class BertForLatentConnector(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None):
        ...
        # input head_mask has shape [num_heads] or [num_hidden_layers x num_heads]
        # and head_mask is converted to shape [num_hidden_layers x batch x num_heads x seq_length x seq_length]
        # print("BERT | head mask", head_mask)
        if head_mask is not None:
            if head_mask.dim() == 1:
                head_mask = head_mask.unsqueeze(0).unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
                head_mask = head_mask.expand(self.config.num_hidden_layers, -1, -1, -1, -1)
        ...
        embedding_output = self.embeddings(input_ids, position_ids=position_ids, token_type_ids=token_type_ids)
        # print("BERT | embedding output shape", embedding_output.shape)
        encoder_outputs = self.encoder(embedding_output,
                                       extended_attention_mask,
                                       head_mask=head_mask)

        # print("BERT | len(encoder_outputs)", len(encoder_outputs))
        # print("BERT | encoder_outputs[0].shape", encoder_outputs[0].shape)
        # print("BERT | encoder_outputs[1].shape", encoder_outputs[1].shape)
        sequence_output = encoder_outputs[0]
        pooled_output = self.pooler(sequence_output)
        # print("BERT | pooled output shape", pooled_output.shape)

        outputs = (sequence_output, pooled_output,) + encoder_outputs[1:]  # add hidden_states and attentions if they are here

        # print("BERT | encoder_outputs[1:]", encoder_outputs[1:])
