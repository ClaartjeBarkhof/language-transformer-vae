...
def sample_sequence_conditional(model, length, context, past=None, num_samples=1, temperature=1, top_k=0, top_p=0.0,
                                device='cpu', decoder_tokenizer=None):
    # print("sample_sequence_conditional | context", context)
    # print("sample_sequence_conditional | past.shape (= latent)", past.shape)
    ...
    with torch.no_grad():
        while True:
            ...
            # print("sample_sequence_conditional | generated in while loop", generated)
            # print("sample_sequence_conditional | past.shape in while loop", past.shape)
            outputs = model(
                **inputs)  # Note: we could also use 'past' with GPT-2/Transfo-XL/XLNet (cached hidden-states)
            # print("sample_sequence_conditional | outputs[0].shape in while loop", outputs[0].shape)
            next_token_logits = outputs[0][0, -1, :] / temperature
            # print("sample_sequence_conditional | next_token_logits.shape in while loop", next_token_logits.shape)
            filtered_logits = top_k_top_p_filtering(next_token_logits, top_k=top_k, top_p=top_p)
            # print("sample_sequence_conditional | filtered_logits.shape in while loop", filtered_logits.shape)
            next_token = torch.multinomial(F.softmax(filtered_logits, dim=-1), num_samples=1)
            # print("sample_sequence_conditional | next_token in while loop", next_token)
            generated = torch.cat((generated, next_token.unsqueeze(0)), dim=1)

            # pdb.set_trace()
            if next_token.unsqueeze(0)[0, 0].item() == decoder_tokenizer.encode('<EOS>')[0]:
                # print("sample_sequence_conditional | EOS token found!")
                break
...
def latent_code_from_text(text, tokenizer_encoder, model_vae, device):
    ...
    with torch.no_grad():
        ...
        # print("latent code is taken to be the mean!")
        coded_length = len(tokenized1)
        # print("len(tokenized1)", len(tokenized1))
        return latent_z, coded_length


def text_from_latent_code(latent_z, model_vae, device, top_k, top_p, temperature,
                          tokenizer_decoder):

    # print("text_from_latent_code | latent_z.shape", latent_z.shape)
    past = latent_z
    context_tokens = tokenizer_decoder.encode('<BOS>')

    # print("text_from_latent_code | XXX", tokenizer_decoder.encoder[context_tokens[0]])

    # print("text_from_latent_code | context_tokens", context_tokens)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating interpolation"):
        ...
        with torch.no_grad():
            if sample_interval == 0 or sample_interval == args.total_sents:
                text_x0 = encoder_tokenizer.decode(x0[0, :x_lengths[0, 0]].tolist(), clean_up_tokenization_spaces=True)[
                    0]
                pooled_hidden_fea = model_vae.encoder(x0, attention_mask=(x0 > 0).float())[1]
...
def interpolate(model_vae, tokenizer_encoder, tokenizer_decoder, sent_source, sent_target, device,
                num_interpolation_steps, top_k, top_p, temperature):
    # and then in the main function

    # print("*"*40)
    # print("args.sent_source:", sent_source)
    # print("args.sent_target:", sent_target)
    # print("*"*40)

    latent_z1, coded_length1 = latent_code_from_text(sent_source, tokenizer_encoder, model_vae, device)
    latent_z2, coded_length2 = latent_code_from_text(sent_target, tokenizer_encoder, model_vae, device)

    # print("latent_z1.shape", latent_z1.shape)
    # print("latent_z1.shape", latent_z1.shape)
    # print("coded_length1", coded_length1)
    # print("coded_length2", coded_length2)
    ...
    for step in range(num_steps + 1):
        ...
        text_interpolate = text_from_latent_code(latent_z, model_vae, device,
                                                 top_k, top_p, temperature,
                                                 tokenizer_decoder)
        result[step] = text_interpolate
        print(step+1, text_interpolate)
...
def add_special_tokens_to_decoder(model_decoder, tokenizer_decoder):
...
def main(jupyter=False):
    ...
    train_data_file = '/Experimentation/Optimus/data/datasets/debug_data/train.txt'
    test_data_file = '/Experimentation/Optimus/data/datasets/debug_data/test.txt'
    checkpoint_dir = '/Experimentation/Optimus/output/LM/Snli/768/philly_vae_snli_b1.0_d5_r00.5_ra0.25_length_weighted/checkpoint-31250/'

    play_mode = 'interpolation' # analogy
    sent_source = "a yellow cat likes to chase a long string ."
    sent_target = "two girls walk in the street happily ."
    sent_input = "a brown dog likes to eat long pasta ."

    parser.add_argument("--train_data_file", default=train_data_file, type=str, required=False,
                        help="The input training data file (a text file).")
    parser.add_argument("--eval_data_file", default=test_data_file, type=str,
                        help="An input evaluation data file to evaluate the perplexity on (a text file).")
    parser.add_argument("--checkpoint_dir", default=checkpoint_dir, type=str, required=False,
                        help="The directory where checkpoints are saved.")
    parser.add_argument("--output_dir", default=checkpoint_dir, type=str, required=False,
    ...
    parser.add_argument("--num_interpolation_steps", default=10, type=int,
                        help="Total sentences to test recontruction.")
    parser.add_argument("--play_mode", default=play_mode, type=str,
    ...
    parser.add_argument('--gloabl_step_eval', type=int, default=31250,
    ...
    parser.add_argument("--sent_source", type=str, default=sent_source)
    parser.add_argument("--sent_target", type=str, default=sent_target)
    parser.add_argument("--sent_input", type=str, default=sent_input)
    ...
    checkpoint = torch.load(os.path.join(output_full_dir, 'training.bin'), map_location=torch.device(args.device))
