    ...
    try:
        for epoch in range(args.epochs):
            ...
            for i in np.random.permutation(len(train_data_batch)):
                ...
                # print("loss_rc.shape", loss_rc.shape)
                # print("loss_kl.shape", loss_kl.shape)
                ...
                # print("report num sents", report_num_sents)
                # print("report_rec_loss.shape", report_rec_loss)

                if iter_ % log_niter == 0:
                    #train_loss = (report_rec_loss  + report_kl_loss) / report_num_sents
                    train_loss = report_loss / report_num_sents

                    train_loss_per_word = report_loss / report_num_words
                    logging('epoch: %d, iter: %d, avg_loss: %.4f, kl: %.4f, recon: %.4f,' \
                           'time elapsed %.2fs, kl_weight %.4f train_loss_per_word %.4f' %
