...
def calc_JSD_over_seq(run_name, exp_name, max_batches, batch_size, dataset, n_exp, device="cuda:0"):
    ...
    with torch.no_grad():
        for batch_i, batch in enumerate(val_loader):
            ...
            for exp_i in range(n_exp):
                ...
                for decode_sample_from_prior_mode in [False, True]:
                    ...
                    vae_output = vae_model(input_ids=batch["input_ids"],
                                           ...
                                           # as many as the batch reconstruct, so batch size
            ...
            jsd_prior_posterior = JSD(probs_posterior_batch, probs_prior_batch, log_probs_posterior_batch,
                                      log_probs_prior_batch)
            jsd_posterior_posterior = JSD(probs_posterior_batch, probs_posterior_reordered_batch,
                                          log_probs_posterior_batch, log_probs_posterior_reordered_batch)
